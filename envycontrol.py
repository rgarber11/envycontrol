...
MODESET_FBDEV_CONTENT = '''# Automatically generated by EnvyControl

options nvidia-drm modeset=1 fbdev=1
'''

MODESET_CURRENT_FBDEV_CONTENT = '''# Automatically generated by EnvyControl

options nvidia-current-drm modeset=1 fbdev=1
'''
MODESET_RTD3 = '''options nvidia "NVreg_DynamicPowerManagement=0x0{}"
'''

MODESET_CURRENT_RTD3 = '''options nvidia-current "NVreg_DynamicPowerManagement=0x0{}"
'''
MODESET_MEMORY_OPTIONS = '''options nvidia NVreg_UsePageAttributeTable=1 NVreg_InitializeSystemMemoryAllocations=0
'''
MODESET_CURRENT_MEMORY_OPTIONS = '''options nvidia-current NVreg_UsePageAttributeTable=1 NVreg_InitializeSystemMemoryAllocations=0
'''
...
def graphics_mode_switcher(graphics_mode, user_display_manager, enable_force_comp, coolbits_value, rtd3_value, fbdev_value, use_nvidia_current):
    ...
    if graphics_mode == 'integrated':
        ...
        print(f"Enable fbdev in modesetting: {fbdev_value}")
        cleanup()
        ...
        file_builder = StringIO()
        if fbdev_value is None:
            if use_nvidia_current:
                file_builder.write(MODESET_CURRENT_CONTENT)
            else:
                file_builder.write(MODESET_CONTENT)
        else:
            if use_nvidia_current:
                file_builder.write(MODESET_CURRENT_FBDEV_CONTENT)
            else:
                file_builder.write(MODESET_FBDEV_CONTENT)
        if rtd3_value is None:
            if use_nvidia_current:
                file_builder.write(MODESET_CURRENT_MEMORY_OPTIONS)
                create_file(MODESET_PATH, file_builder.getvalue())
            else:
                file_builder.write(MODESET_MEMORY_OPTIONS)
                create_file(MODESET_PATH, file_builder.getvalue())
        else:
            # setup rtd3
            if use_nvidia_current:
                file_builder.write(MODESET_CURRENT_RTD3.format(rtd3_value))
                file_builder.write(MODESET_CURRENT_MEMORY_OPTIONS)
                create_file(
                    MODESET_PATH, file_builder.getvalue())
            else:
                file_builder.write(MODESET_RTD3.format(rtd3_value))
                file_builder.write(MODESET_MEMORY_OPTIONS)
                create_file(MODESET_PATH, file_builder.getvalue())
        ...
        if use_nvidia_current:
            if fbdev_value:
                create_file(MODESET_PATH, MODESET_CURRENT_FBDEV_CONTENT + MODESET_CURRENT_MEMORY_OPTIONS)
            else:
                create_file(MODESET_PATH, MODESET_CURRENT_CONTENT + MODESET_CURRENT_MEMORY_OPTIONS)
        else:
            if fbdev_value:
                create_file(MODESET_PATH, MODESET_FBDEV_CONTENT + MODESET_MEMORY_OPTIONS)
            else:
                create_file(MODESET_PATH, MODESET_CONTENT + MODESET_MEMORY_OPTIONS)
...
def main():
    ...
    parser.add_argument('--fbdev', action='store_true',
                        help='Set nvidia-drm.fbdev=1 within the driver settings for better performance with Wayland.')
        ...
        with CachedConfig(args).adapter():
            if args.switch:
                ...
                graphics_mode_switcher(
                    args.switch, args.dm,
                    args.force_comp, args.coolbits, args.rtd3, args.fbdev, args.use_nvidia_current
                )
